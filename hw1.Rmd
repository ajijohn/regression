...
mri%>% filter(!is.na(packyrs) & (packyrs > 10 & packyrs <= 10 )) %>% 
...
mri%>% filter(!is.na(packyrs) & (packyrs > 0 & packyrs <= 10 )) %>% group_by(male)  %>% summarise(mg=n())

mri%>% filter(!is.na(packyrs) & (packyrs > 0 & packyrs <= 10 )) %>% group_by(stroke)  %>% summarise(ms=n())

mri%>% filter(!is.na(packyrs) & (packyrs > 0 & packyrs <= 10 )) %>% group_by(chf)  %>% summarise(mchf=n())

mri%>% filter(!is.na(packyrs) & (packyrs > 0 & packyrs <= 10 )) %>% group_by(chd)  %>% summarise(mchd=n())

mri%>% filter(!is.na(packyrs) & (packyrs > 0 & packyrs <= 10 )) %>% group_by(death)  %>% summarise(mcdt=n())
...
|:--------------|:-------------|:----------------|:----------------|:-------------------|:--------------|
|   Age(yrs)         |  75.3(5.9, 66\-99)          |  74.8(5.8, 68\-95)     |                 |                    |
|   Male(\%)          |  38\%                      |    63\%                |                 |                    |
|   Weight(lbs)          |  155.6(30.2, 74\-264)   |  162.2(27.3, 107\-242) |                 |                    |
|   Prior Stroke Diagnosis(\%)          |  3\%     |    2\%                 |                 |                    |
|   Prior CHF Diagnosis(\%)         |  5\%         |    4\%                 |                 |                    |
|   Prior CHD  Diagnosis(\%)        |  9\%         |    6\%                 |                 |                    |
|   Death(\%)          |  15\%                     |    12\%                |                 |                    |
